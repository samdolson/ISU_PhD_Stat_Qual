...
x_{2i} =
\begin{cases}
1, & \text{student is female} \\
0, & \text{student is male}
\end{cases}
$$
...
- If we fit the “true” model, Model B, then:
  - bias $= 0$
  - variance $= \sum_i \mathrm{Var}(\hat{Y}_i) = \sigma^2 (k + 1 + \dim(Z))$
...
$$
C_p = \frac{SS_{\text{error}}}{\hat{\sigma}^2} - \left[n - 2(k + 1)\right]
$$
...
$$
Y_i = \mu_{Y \mid x} + \varepsilon_i \quad \text{where } \varepsilon_i \text{ i.i.d. } N(0, \sigma^2)
$$
...
- $\mu_{Y \mid x}$ is a linear function of $\mathbf{x}$
- Homogeneous error variance: $\mathrm{Var}(\varepsilon_i) = \sigma^2$
- Normally distributed errors: $\varepsilon_i$ i.i.d. $N(0, \sigma^2)$

#### Weighted Least Squares

- Assume $\mathrm{Var}(\varepsilon_i) = \sigma_i^2$ for $i = 1, \ldots, n$
...
$$
\mathrm{Var}(Y_i) = \sigma^2 / n_i \quad \text{and} \quad w_{ii} = n_i
$$

- If the $i$th observation is a total of $n_i$ observations, then

$$
\mathrm{Var}(Y_i) = n_i \sigma^2 \quad \text{and} \quad w_{ii} = 1 / n_i
$$

- If variance is proportional to some predictor $x_j$, then

$$
\mathrm{Var}(Y_i) = x_{ij} \sigma^2 \quad \text{and} \quad w_{ii} = 1 / x_{ij}
$$
...
$$
\mathrm{MSE}_{\text{Validation}} =
\frac{1}{n_{\text{test}}}
\sum_{i=1}^{n_{\text{test}}}
\left(Y_i - \hat{Y}_i\right)^2
$$

- Should be approximately equal to $\mathrm{MSE}_{\text{Training}}$ from selected model
- $\mathrm{MSE}_{\text{Validation}}$ will be substantially larger if model is overfit to the training sample
- Use as model selection technique:
  - Fit many models to the training sample
  - Compute $\mathrm{MSE}_{\text{Validation}}$ for the validation sample for each selected model
...
$$
\mathrm{Var}(\mathbf{Y}) = \mathrm{Var}(\boldsymbol{\varepsilon}) = \sigma^2 I
$$
...
|----------|------|----------------|-------------|
| Model | $2$ | $\displaystyle \sum_{i=1}^{5}(\hat{Y}_i - \bar{Y})^2 = \mathbf{Y}^T (P_X - P_1)\mathbf{Y}$ | $\displaystyle \frac{1}{2}SS_{\text{model}}$ |
| Error | $2$ | $\displaystyle \sum_{i=1}^{5}(Y_i - \hat{Y}_i)^2 = \mathbf{Y}^T (I - P_X)\mathbf{Y}$ | $\displaystyle \frac{1}{2}SS_{\text{error}}$ |
| Total | $4$ | $\displaystyle \sum_{i=1}^{5}(Y_i - \bar{Y})^2 = \mathbf{Y}^T (I - P_1)\mathbf{Y}$ |  |
...
$$
Q(\mathbf{b})
= \sum_{i=1}^n \left(Y_i - \mathbf{x}_i^T \mathbf{b}\right)^2
...
$$
0 = \frac{\partial Q(\mathbf{b})}{\partial b_j}
= 2 \sum_{i=1}^n \left(Y_i - \mathbf{x}_i^T \mathbf{b}\right) x_{ij}.
$$

These equations are expressed in matrix form as

$$
\mathbf{0} = X^T (\mathbf{Y} - X\mathbf{b})
= X^T \mathbf{Y} - X^T X \mathbf{b},
$$
...
If $X_{n \times k}$ has full column rank, $\mathrm{rank}(X) = k$, then:
...
Define the projection matrix $P_X$ to be
...
For a linear model $E(Y) = X\beta$, the parameter vector $\beta$ is *identifiable* if
...
#### Example: One-Way ANOVA Cell Means Model

Write: 

$\beta_1 = \left( \beta_1, \beta_2, \beta_3 \right)^{\top}$

$\beta_2 = \left( \beta_1^*, \beta_2^*, \beta_3^*\right)^{\top}$
...
An *estimable function* is a linear function of identifiable parameters.
...
- Each element of $\beta$ is estimable if and only if $\operatorname{rank}(X) = k$, where $k$ is the number of columns in $X$.
- Every $c^T\beta$ is estimable if and only if $\operatorname{rank}(X) = k$, the number of columns in $X$.
...
- No assumption is made about $\operatorname{Var}(Y)$ or $\operatorname{Var}(\varepsilon)$ or the shape of the distribution of $Y$ or $\varepsilon$.
...
Let $\mathbf{Y}$ be an $n$-dimensional random vector and let $\mathbf{A}$ be a non-random $n \times n$ matrix.  
A *quadratic form* is a random variable defined by
$$
...
If $E(\mathbf{Y}) = \boldsymbol{\mu}$ and $\mathrm{Var}(\mathbf{Y}) = \boldsymbol{\Sigma}$, then

$$
E\!\left(\mathbf{Y}^\top \mathbf{A}\mathbf{Y}\right)
= \mathrm{tr}(\mathbf{A}\boldsymbol{\Sigma}) + \boldsymbol{\mu}^\top \mathbf{A}\boldsymbol{\mu}.
$$
...
$$
\delta = \frac{1}{2}\boldsymbol{\mu}^\top \boldsymbol{\mu}
= \frac{1}{2}\sum_{i=1}^n \mu_i^2.
$$

#### Distribution of Quadratic Forms

Let $\mathbf{A}$ be an $n \times n$ symmetric matrix with $\mathrm{rank}(\mathbf{A})$, and let
...
$$
\mathbf{Y}^\top \mathbf{A}\mathbf{Y}
\sim \chi^2_{\mathrm{rank}(\mathbf{A})}
\left( \frac{1}{2}\boldsymbol{\mu}^\top \mathbf{A}\boldsymbol{\mu} \right).
$$
...
$$
\mathbf{A}_i \boldsymbol{\Sigma} \mathbf{A}_j = \mathbf{0}
\quad \text{for all } i \neq j,
$$
...
$$
\mathbf{Y}^\top \mathbf{A}_1 \mathbf{Y}, \;
\mathbf{Y}^\top \mathbf{A}_2 \mathbf{Y}, \;
\ldots, \;
\mathbf{Y}^\top \mathbf{A}_p \mathbf{Y}
$$
...
$$
W_1 \sim \chi^2_{n_1}(\delta_1), \quad
W_2 \sim \chi^2_{n_2},
$$
